...
func init() {
	events.RegisterSlashCommand(&HelpCommandSlash{})
	events.RegisterPrefixCommand(&HelpCommandPrefix{})
}
...
func loadHelpLib() (*library.CommandLib, error) {
	...
	path := fmt.Sprintf("Modules/Locales/%s/Commands/Help.yaml", config.AppConfig.Bot.Lang)
